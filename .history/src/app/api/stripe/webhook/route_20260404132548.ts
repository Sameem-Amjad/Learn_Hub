...
async function markPastDue(invoice: Stripe.Invoice) {
  ...
  const subscriptionId =
    typeof invoiceObject.subscription === "string"
      ...
      : invoiceObject.subscription?.id ?? invoiceObject.parent?.subscription_details?.subscription;
  if (!subscriptionId) {
    return;
  }
  ...
  try {
    switch (event.type) {
      ...
      case "invoice.payment_succeeded": {
        ...
        const subscriptionId = invoiceObject.subscription ?? invoiceObject.parent?.subscription_details?.subscription;
