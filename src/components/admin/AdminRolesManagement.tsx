...
export function AdminRolesManagement() {
  ...
  const [busyAction, setBusyAction] = useState<"assign" | "remove" | null>(null);
  const pageSize = 10;
  ...
  async function assignAdmin(userId: string) {
    ...
    if (!res.ok) {
      ...
      toast.error(payload.error ?? "Failed to assign admin role");
      setBusyUserId(null);
      setBusyAction(null);
      return;
    }
  ...
  async function removeAdmin(userId: string) {
    ...
    if (!res.ok) {
      const payload = (await res.json()) as { error?: string };
      toast.error(payload.error ?? "Failed to remove admin role");
      setBusyUserId(null);
      setBusyAction(null);
      return;
    }
              ...
              <div className="flex items-center gap-2">
                ...
                {user.is_admin ? (
                  <Button
                    ...
                    isLoading={busyUserId === user.id && busyAction === "remove"}
                  ...
                  <Button
                    ...
                    isLoading={busyUserId === user.id && busyAction === "assign"}
